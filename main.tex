...
\begin{titlepage}
    ...
    \vspace{6cm}
    \large \textbf{RELATÓRIO PARCIAL DE INICIAÇÃO CIENTÍFICA} \\
    \vspace{1cm}
    \Large \textbf{Predição da avaliação funcional de especialistas em lesão de LCA via \textit{Markerless} e \textit{Machine Learning} por meio de um teste Pliométrico Unipodal Multiplanar} \\
    \vspace{5cm}
    \large \textbf{Aluna:} Anna Carolina Magalhães Leonel \\
    \large \textbf{Orientador:} Prof. Dr. Paulo Roberto Pereira Santiago \\
    \vspace{0.5cm}
     \large \textbf{Laboratório de Biomecânica e Engenharia Aplicada - LABE/USP}\\
     \vspace{0.5 cm}
    \textbf{Vigência:} #2025/12112-5 (FAPESP) \\
    \vfill
    Ribeirão Preto \\
    2026
\end{titlepage}
...
\noindent Este relatório parcial apresenta o andamento do projeto de iniciação científica focado na predição da avaliação funcional de especialistas em pacientes pós-reconstrução de LCA. Utilizando cinemática \textit{markerless} bia \textit{MediaPipe} e modelos de \textit{Machine Learning}, o projeto visa objetivar a tomada de decisão clínica no retorno ao esporte. Até o presente momento, foram realizadas 6 sessões de coleta de dados, capacitação nas ferramentas de visão computacional e planejamento completo do estágio de pesquisa internacional (BEPE) na \textit{University of North Florida}. O progresso atual indica o cumprimento satisfatório das metas iniciais, preparando o terreno para a validação transcultural prevista para o próximo semestre. 

\vspace{0.5cm}
\noindent\textbf{Palavras-chave}: Biomecânica, \textit{markerless}, LCA, \textit{Machine Learning}, BEPE.
...
\noindent\textbf{Projeto:} Predição da avaliação funcional de especialistas em lesão de LCA via \textit{Markerless} e \textit{Machine Learning} por meio de um teste Pliométrico Unipodal Multiplanar.\\
\textbf{Orientador:} Prof. Dr. Paulo Roberto Pereira Santiago.\\
\textbf{Aluna:} Anna Carolina Magalhães Leonel.\\
\textbf{Fomento:} FAPESP (Processo \#2025/12112-5).\\
Laboratório de Biomecânica e Engenharia Aplicada - LABE\\
Faculdade de Medicina de Ribeirão Preto - FMRP/USP\\
\\
Colaborador Nacional: Prof. Dr. Rodrigo Salim\\
Doutorando Colaborador: Rafael Luiz Martins Monteiro\\
Colaborador Internacional: Prof. Dr. Guilherme Manna Cesar (UNF)\\
Dra. Sherry Pinkstaff (UNF)\\
...
No período de vigência da bolsa até o momento (março de 2026), as atividades concentraram-se na fundamentação teórica, capacitação técnica e execução das coletas experimentais.

\subsection{Capacitação e Fundamentação Teórica}
A aluna dedicou-se à revisão bibliográfica sistemática sobre biomecânica do LCA e inteligência artificial. Foram dominadas as seguintes ferramentas:
\begin{itemize}
    \item \textbf{\textit{MediaPipe} via \textit{vailá}}: Extração de coordenadas 2D e 3D sem marcadores.
    \item \textbf{Python (\textit{Scikit-learn}, \textit{Pandas})}: Processamento de dados e implementação de modelos de classificação.
    \item \textbf{\LaTeX}: Escrita científica estruturada.
\end{itemize}

\subsection{Atividades Experimentais e Coleta de Dados}
Até o momento, foram realizadas 6 sessões de coleta de dados na clínica do colaborador Prof. Dr. Rodrigo Salim. 
\begin{itemize}
    \item \textbf{Setup}: Utilização de 3 câmeras GoPro Hero 10 a 240fps.
    \item \textbf{Calibração}: Uso de marcadores estáticos para 3D-DLT.
    \item \textbf{Desafios}: Ocorreram algumas desistências de participantes, porém o cronograma de recrutamento foi ajustado para manter o $N$ necessário para o treinamento dos modelos.
\end{itemize}

\subsection{Processamento e Resultados Preliminares}
Os dados das 6 coletas iniciais já passaram pela \textit{pipeline} de extração cinemática. Foram calculadas variáveis como valgo dinâmico e picos de flexão. Estas variáveis estão sendo integradas para a fase de treinamento dos classificadores supervisionados.

\subsection{Internacionalização e Planejamento BEPE}
Uma atividade central deste período foi a submissão e estruturação estratégica do estágio de pesquisa no exterior (\textit{University of North Florida}). A aluna já obteve o aceite formal da instituição anfitriã para o período de junho a dezembro de 2026.

\section{DESEMPENHO ACADÊMICO}
No período vigente, o desempenho acadêmico da discente tem sido satisfatório, com pleno cumprimento das disciplinas curriculares da graduação em Fisioterapia. 
\begin{itemize}
    \item \textbf{Média Ponderada}: \textit{[Inserir Média Aqui]}
    \item \textbf{Histórico}: Sem reprovações e com engajamento em atividades de extensão e ligas acadêmicas do campus.
\end{itemize}

\section{DESENVOLVIMENTO DAS ATIVIDADES}
As atividades previstas para o primeiro semestre da bolsa foram integralmente cumpridas, com destaque para a antecipação das coletas e o sucesso na articulação internacional.
...
\begin{table}[h!]
\centering
\caption{Cronograma de atividades (Vigência 2025-2026)}
\label{tab:cronograma_destacado}
\begin{tabular}{>{\raggedright\arraybackslash}m{8cm}*{12}{c}}
\toprule
...
\midrule
Capacitação (CV, ML, \LaTeX) & \cellcolor{gray!30} & \cellcolor{gray!30} & & & & & & & & & & \\
Recrutamento e Coleta de Dados & & & \cellcolor{gray!30} & \cellcolor{gray!30} & \cellcolor{gray!30} & \cellcolor{gray!30} & & & & & & \\
Processamento Avançado (OpenSim) & & & & & & & \cellcolor{gray!30} & \cellcolor{gray!30} & & & & \\
Treinamento e Validação Cross-Cultural (BEPE) & & & & & & & & & \cellcolor{gray!30} & \cellcolor{gray!30} & \cellcolor{gray!30} & \cellcolor{gray!30} \\
Escrita Científica e Relatório Final & & & & & & & & & & & \cellcolor{gray!30} & \cellcolor{gray!30} \\
\bottomrule
\end{tabular}
\end{table}

\section{PRÓXIMAS ETAPAS}
As próximas fases do projeto contemplam:
\begin{itemize}
    \item \textbf{Integração OpenSim}: Extração de momentos articulares e forças musculares.
    \item \textbf{Validação Internacional}: Aplicação do modelo em amostra americana na UNF.
    \item \textbf{Publicação}: Submissão de artigo em periódico de alto impacto na área de biomecânica.
\end{itemize}
